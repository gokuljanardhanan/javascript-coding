const knapSackBruteForce = (profit, weight, capacity) => {
...
const knapSackHelper = (index, profit, weight, capacity) => {
  ...
  if (newCapacity >= 0) {
    let profit =
      profit[index] + knapSackHelper(index + 1, profit, weight, newCapacity);
    maxProfit = Math.max(maxProfit, profit);
  }
  return maxProfit;
};

const knapSackMemo = (profit, weight, capacity) => {
...
const knapSackHelperMemo = (index, profit, weight, capacity, cache) => {
  ...
  if (newCapacity >= 0) {
    let profit =
      profit[index] +
      knapSackHelper(index + 1, profit, weight, newCapacity, cache);
